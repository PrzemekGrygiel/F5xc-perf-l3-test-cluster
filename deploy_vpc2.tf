...
resource "aws_subnet" "pg-vpc2-inside-az-b-subnet" {
  vpc_id                  = "${aws_vpc.pg-vpc2.id}"
  cidr_block              = "10.131.3.0/24"
  availability_zone       = format("%sb", var.aws_region)
}
resource "aws_subnet" "pg-vpc2-inside-az-c-subnet" {
  vpc_id                  = "${aws_vpc.pg-vpc2.id}"
...
resource "aws_subnet" "pg-vpc2-outside-az-b-subnet" {
  vpc_id                  = "${aws_vpc.pg-vpc2.id}"
  cidr_block              = "10.131.5.0/24"
  availability_zone       = format("%sb", var.aws_region)
}
resource "aws_subnet" "pg-vpc2-outside-az-c-subnet" {
  vpc_id                  = "${aws_vpc.pg-vpc2.id}"
...
resource "aws_subnet" "pg-vpc2-workload-az-b-subnet" {
  vpc_id                  = "${aws_vpc.pg-vpc2.id}"
  cidr_block              = "10.131.4.0/24"
  availability_zone       = format("%sb", var.aws_region)
}
resource "aws_subnet" "pg-vpc2-workload-az-c-subnet" {
  vpc_id                  = "${aws_vpc.pg-vpc2.id}"
